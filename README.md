...
hexadecimal text fields in ordinary `FormData`, using the same externally reachable HTTP path as
synchronization requests. Only the current slice is encoded; the complete file is never read into
one browser-side buffer. The server also accepts raw and legacy multipart-file chunks from cached
clients. The chunk and complete-file limits have one server-side source of truth and
